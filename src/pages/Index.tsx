...
useEffect(() => {
    ...
    setMatches(prev =>
      prev.map(m => {
        if (m.status === "live" && m.match_date) {
          const kickoff = parseAsWIB(m.match_date).getTime();
          const diff = Math.floor((Date.now() - kickoff) / 60000);
          const liveMinute = diff >= 0 ? diff : 0;
          // hindari update kalau nilainya sama -> cegah re-render sia-sia
          if (m.liveMinute === liveMinute) return m;
          return { ...m, liveMinute };
        }
        return m;
      })
  ...
  updateLiveMinutes();
  const id = setInterval(updateLiveMinutes, 60000);
  ...
  function parseAsWIB(dateString: string | null) {
    if (!dateString) return new Date(); // fallback ke sekarang
